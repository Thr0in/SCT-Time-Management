...
class STCHeader(tk.Frame):
    def __init__(self, parent, main=None):
        ...
        tk.Label(self.logo, text='STC', font=gui_constants.EXTRA_LARGE).pack(side='top')
        tk.Label(self.logo, text='Time Management System', font=gui_constants.BOLD).pack(side='top')


        self.menu = tk.Frame(self, relief="solid", padx=20)
        self.menu.pack(side='right', expand=True, fill=tk.BOTH)

        logout = tk.Button(self.menu, text="Logout", command=lambda: self.logout(), font=gui_constants.BOLD)
        logout.pack(side='right')

        employee = tk.Frame(self.menu)
        employee.pack(side='right')

        tk.Label(employee, textvariable=self.employee_name, font=gui_constants.LARGE).pack(side='top')
