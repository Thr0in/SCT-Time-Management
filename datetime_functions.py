    ...
    def merge_date_and_time_to_datetime(self, date, time): 
        ...
        if isinstance(time, str):
            try:
                ...
                raise ValueError("Time is a string and cannot be converted to datetime. Therefore Date and time cannot be merged to datetime.")
                
        if isinstance(date, str):
            try:
                date = self.convert_string_to_date(date)
            except ValueError:
                raise ValueError("Date is a string and cannot be converted to datetime. Therefore Date and time cannot be merged to datetime.")
        ...
        # If both are complete datetime objects (each has a date and a time part):
            ...
            raise TypeError("Both date and time must be datetime objects or valid date/time strings")
    
    #------------------------------------------------------------------------------
    
    def get_time_difference(self, start_time, end_time): 
        """ Calculates time difference between two datetimes """
